...
export function logTimestampComparison(
  ...
  console.log(`⏰ [TimestampDebug] ${label}:`, {
    ...
    pushbulletApiISO: timestamps.pushbulletApiSeconds
      ? new Date(timestamps.pushbulletApiSeconds * 1000).toISOString()
      : undefined,
    javascriptMs: timestamps.javascriptMs,
    javascriptISO: timestamps.javascriptMs
      ? new Date(timestamps.javascriptMs).toISOString()
      : undefined,
    lastSeenSeconds: timestamps.lastSeenSeconds,
    lastSeenISO: timestamps.lastSeenSeconds
      ? new Date(timestamps.lastSeenSeconds * 1000).toISOString()
      : undefined,
    lastSeenMs: timestamps.lastSeenMs,
    lastSeenMsISO: timestamps.lastSeenMs
      ? new Date(timestamps.lastSeenMs).toISOString()
      : undefined,
